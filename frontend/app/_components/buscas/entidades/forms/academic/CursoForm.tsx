...
export default function CursoForm({
    ...
    // // adiciona cursoId a updateCursoAction
    const updateCursoWithId = updateCursoAction.bind(null, curso.id);

    const [state, formAction, pending] = useActionState(updateCursoWithId, { error: '', success: false, message: ''});

    // // campos para edição
    const [editNome, setEditNome] = useState(curso.nome);
    const [editCampusId, setEditCampusId] = useState(curso.campusId);
    ...
    useEffect(() => {
        if (state.success) {
            ...
            const timer = setTimeout(() => {
                ...
                router.push(`/buscar-entidades/cursos/${curso.id}`);
            }, 3000);
  ...
  return (
    <div className="rounded-sm flex flex-col bg-white flex-1">
        ...
        <div className={`flex-1 gap-10 flex mt-7 justify-around ${successMessage ? 'hidden': ''}`}>
            ...
            <Button
            aria-label="Atualizar dados do curso"
            style={{ backgroundColor: 'var(--color-tertiary)'}}
            className="cursor-pointer"
            disabled={editNome === curso.nome && editCampusId === curso.campusId}
            type="submit">
        ...
        {!successMessage ? '' : (
            <div className="mt-5 flex flex-col justify-center">
                <CheckCircleIcon color='green' className="h-8" />
                 <p className="text-center font-semibold" style={{color: 'var(--color-secondary)'}}>Curso atualizado com sucesso!</p>
