...
import Image from "next/image"; // <-- Adicionado o import correto que estava faltando no seu local!
        ...
        <Navbar style={{ backgroundColor: 'var(--color-primary)' }}>
            <div className="flex flex-row flex-5 items-center">
                ...
                <div className="flex flex-col">
                    ...
                    >
                        Dashboard - {role.slice(0, 1).toUpperCase() + role.slice(1)}
                    </h1>
                    ...
                    <div>
                        <Link 
                            ...
                            style={{ backgroundColor: `${index === 1 ? 'var(--tab-active)' : 'var(--color-primary)'}`, color: 'var(--light-color)' }}
                        >
                            Home
                        </Link>
                        {role === 'admin' ? (
                            <Link 
                                href='/auditoria' 
                                className="mr-5 font-semibold mt-1 p-0.5 pl-2 pr-2 self-baseline rounded" 
                                style={{ backgroundColor: `${index === 2 ? 'var(--tab-active)' : 'var(--color-primary)'}`, color: 'var(--light-color)' }}
                            >
                                Auditoria
                            </Link>
                        ) : ''}
